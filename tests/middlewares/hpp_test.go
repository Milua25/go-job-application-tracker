...
func newHPPTestRouter(opts middlewares.HPPOptions) *gin.Engine {
	router := gin.New()
	router.Use(opts.Hpp())
...
func TestHPP_QueryDeduplication(t *testing.T) {
	opts := middlewares.HPPOptions{CheckQuery: true, Whitelist: []string{"foo"}}
...
func TestHPP_QueryFilterDisabled(t *testing.T) {
	opts := middlewares.HPPOptions{CheckQuery: false, Whitelist: []string{"foo"}}
	router := newHPPTestRouter(opts)
